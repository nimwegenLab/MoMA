...
if [[ $# -gt 1 ]]; then  # arguments were passed; try to parse the paths
  path_args=("-i", "--infolder", "-infolder", "-o", "--outfolder", "-outfolder", "-p", "--props", "-props", "-rl", "--reload", "-reload") # arguments that are followed by a path
  mount_string=""
  while :
  do
...
if [[ -n ${GRB_LICENSE_FILE} ]]; then
  ...
  printf "ERROR: Could not determine path to Gurobi license file. Variable not set: GRB_LICENSE_FILE"
  exit 1
fi
...
if [[ -n ${HOME} ]]; then
  ...
  printf "ERROR: Could not determine home-directory path. Variable not set: HOME"
  exit 1
fi
...
case $containerization in
 "singularity")
  ...
  "docker")
    ...
    eval docker run -it --rm --user $(id -u):$(id -g) --env "HOME" --env "GRB_LICENSE_FILE" "${x_forwarding_options}" "${mount_string}" "${CONTAINER_TAG}" "${args_copy}"
    ;;
esac
