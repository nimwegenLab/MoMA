...
class RangeSliderUI extends BasicSliderUI {
    ...
    private void paintLowerThumb(final Graphics g) {
		...
		final int w = knobBounds.width / 2;
    ...
    class RangeTrackListener extends TrackListener {
        ...
        public void mousePressed(final MouseEvent e) {
			...
			// Handle click on slider itself
			int value = 0;
			switch ( slider.getOrientation() ) {
			case SwingConstants.VERTICAL:
				if ( !lowerPressed && !upperPressed ) {
					value = valueForYPosition( e.getY() );
				}
				break;
			case SwingConstants.HORIZONTAL:
				if ( !lowerPressed && !upperPressed ) {
					value = valueForXPosition( e.getX() );
				}
				break;
			}
			final int lowerValue = slider.getValue();
			final int newExtent =
					Math.min(
							Math.max( 0, value - lowerValue ),
							slider.getMaximum() - lowerValue );
