 ...
 * @author jug
 */
public class HtmlOverviewExporterWriter {
    ...
    public HtmlOverviewExporterWriter(final MoMAGui gui, final File htmlFile, final String imgpath, final int startFrame, final int endFrame) {
        this.gui = gui;
    ...
    public void run() {
        ...
        try {
            ...
            for (int frameIndex = startFrame; frameIndex <= endFrame; frameIndex++) {
                gui.setCenterTime(frameIndex);
                try {
                    String fn1 = String.format("/glf_%03d.png", frameIndex);
...
//					gui.imgCanvasActiveCenter.exportSegmentationImage( imgpath + fn2 );
                    row1.append("			<th><font size='+2'>t=").append(frameIndex).append("</font></th>\n");
                    row2.append("			<td><img src='").append(basename).append(fn1).append("'></td>\n");
//					row3 += "			<td><img src='./imgs" + fn2 + "'></td>\n";

                    if (frameIndex < endFrame) {
                        fn1 = String.format("/assmnts_%03d.png", frameIndex);
